...
class FacturaModel {
    ...
    private static function dropRestrictiveUniqueIndexes(PDO $pdo): void {
        // El esquema PostgreSQL generado para DTE no crea UNIQUE restrictivos
        // sobre columnas fiscales, asi que no hace falta desmontarlos aqui.
    }

    private static function ensureIndex(PDO $pdo, string $indexName, string $columns): void {
        try {
            $pdo->exec("CREATE INDEX IF NOT EXISTS {$indexName} ON dte_facturas ({$columns})");
        } catch (Throwable $exception) {
            // Si no se puede crear, dejamos que PostgreSQL reporte el error real mas adelante.
        }
    }
